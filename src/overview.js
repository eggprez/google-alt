...
import { renderAnswer, splitSources, parseVerification, citedSources, hostOf } from './render.js';
...
async function execute(run) {
  ...
  if (results && results.length) {
    ...
    run.flushSnapshot();
    run.setQuick(renderAnswer(answer, sources));
    ...
    if (config.claude.verify) {
      ...
      try {
        ...
        run.setChecked(renderAnswer(answer, sources), verification);
      } catch (e) {
        verification = { status: 'failed', error: e.message };
        run.setChecked(run.quickHtml, verification);
      }
    }
  } else {
  ...
  return {
    html: renderAnswer(answer, sources),
    answer,
...
export async function runFollowup({ q, question, history = [], onStatus = () => {}, onSnapshot = () => {} }) {
  ...
  // The panel keeps every source the conversation has cited so far.
  base.sources = all;
  base.at = Date.now();
  return { html: renderAnswer(answer, all), answer, sources: all, cost: r.cost, turns: r.turns };
}
